...
const dimensions=(file,label=file)=>{const data=fs.readFileSync(file);assert.ok(data.length>=24,`${label} must contain a complete PNG header`);assert.equal(data.subarray(0,8).toString("hex"),"89504e470d0a1a0a",`${label} must be a PNG`);return[data.readUInt32BE(16),data.readUInt32BE(20)]}
